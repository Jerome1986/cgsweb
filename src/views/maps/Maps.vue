...
const cateGetAll = async () => {
  ...
  // 一级分类
  topCate.value = res.data.list.filter((item) => item.type === '贴图')
...
const handleTopCate = (id) => {
  ...
  // 获取一级分类素材
  materialRef.value.materialDataGet('贴图', currentMapsCateId.value)
  // 重置二级分类的高亮状态
  if (subCateRef.value) {
...
<template>
  <div class="mapsPage">
    ...
    <!--  素材  -->
    <SourceMaterial ref="materialRef" type="贴图" :currentCateId="currentMapsCateId"></SourceMaterial>
  </div>
</template>
