...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // 添加原生文件拖拽方法
  startFileDrag: (filePath, fileType = 'model') => 
  ...
  // 添加获取特定类型文件路径的方法
  getSpecificFilePath: (dirPath, fileType) => 
    ipcRenderer.invoke('get-specific-file-path', dirPath, fileType),

  // 添加准备下载路径的方法
  prepareDownloadPath: (basePath, category) => 
    ipcRenderer.invoke('prepare-download-path', basePath, category),

  // 添加删除文件的功能
  deleteFile: (filePath) => ipcRenderer.invoke('delete-file', filePath),

  // 添加写入路径到文件的功能
  writePathToFile: (filePath, txtFileName) => 
    ipcRenderer.invoke('write-path-to-file', filePath, txtFileName),

  // 添加获取资源路径的方法
  getResourcesPath: () => ipcRenderer.invoke('get-resources-path'),

  // 添加拖拽准备方法
  prepareDragAndDrop: (basePath, fileType) => 
    ipcRenderer.invoke('prepare-drag-and-drop', basePath, fileType),
